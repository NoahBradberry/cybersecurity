...
def add_password():
    ...
    if not account or not site or not username or not password:
...
def search_password():
    ...
    if account in passwords and site in passwords[account]:
        info = passwords[account][site]
        username_entry.delete(0, tk.END)
        username_entry.insert(0, info["username"])
        password_entry.delete(0, tk.END)
        password_entry.insert(0, info["password"])
    else:
        messagebox.showinfo("Not found", "No password saved for that account + site")
...
def delete_password():
    ...
    if account in passwords and site in passwords[account]:
        ...
        messagebox.showerror("Error", "Account or site not found")
...
def update_listbox():
    ...
    for account, sites in passwords.items():
        for site in sites:
            saved_list.insert(tk.END, f"{account} → {site}")
...
passwords = load_passwords()  # format: {account: {site: {username, password}}}
